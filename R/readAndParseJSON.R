...
#'
#' @param file (vector of character strings; no default) File path(s) to trace
#' file.
...
readAndParseJSON <- function(file) {
  ...
  is_metadata_line <- function(line) {
    tryCatch({
      json <- fromJSON::fromJSON(line, simplifyVector = TRUE)
  ...
  for (line in json_lines) {
    if (!jsonlite::is_metadata_line(line)) {
